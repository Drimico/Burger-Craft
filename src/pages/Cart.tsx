...
const Cart = () => {
  ...
  const { craftedBurgers, removeCraftedBurger, updateCraftedBurgerQty } = useCraftStore();
  return (
    <div className="flex flex-col w-screen min-h-screen mt-[160px] items-center justify-evenly p-10 text-2xl relative text-white">
      <Link to="/" className="flex items-center gap-2 md:text-3xl text-2xl cursor-pointer hover:text-emerald-600 w-fit absolute top-[5%] left-[10%]">
        <ChevronLeft className="animate-bounceX" size={30} />
        {t("ui.back_to_menu")}
      </Link>
      {cart.length === 0 && craftedBurgers.length === 0 && (
        <div className="absolute text-3xl top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">{t("ui.empty_cart")}</div>
      )}
