...
const Selector = ({ sectionIndex, id, options, title }: SelectorProps) => {
  ...
  return (
    <div className="flex flex-col 1280:text-2xl 300:text-xl 1280:w-90 400:w-70 w-full  h-fit gap-3 ">
      <div className="1280:text-4xl sm:text-3xl 300:text-2xl text-xl">{t(title)}</div>
      {selections.map((item, index) => (
        <SelectorDropMenu key={item.id} options={options} itemId={item.id} optionIndex={index} sectionIndex={sectionIndex} />
      ))}
      {id !== 1 && (
