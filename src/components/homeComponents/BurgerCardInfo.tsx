...
const BurgerCardInfo = ({ checkedAdds, setCheckedAdds, name, index, description, adds, weight, windowWidth }: BurgerCardProps) => {
  ...
  const handleToggleAdd = (id: number) => {
    setCheckedAdds((prev) => prev.map((check, idx) => (idx === id ? !check : check)));
  };
            ...
            <div className="flex flex-col gap-2 h-fit w-full items-center justify-between p-[0px_1rem_1rem_1rem] text-white/80">
              ...
              <div className="flex lg:flex-col w-full justify-center items-center gap-2">
                <div className="flex gap-3 lg:text-2xl text-lg justify-center items-center">
                  <button type="button" onClick={() => setOrders((prev) => Math.max(1, prev - 1))}>
                    <Minus size={20} className="text-red-700 cursor-pointer" />
                  </button>
                  <span>{orders}</span>
                  <button type="button" onClick={() => setOrders((prev) => prev + 1)}>
                    <Plus size={20} className="text-emerald-800 cursor-pointer" />
                  </button>
                </div>
